...
//
//    import Dataset.{imgPre, imgSuf, padVal}
...
//
//    regBegin -- Skip(reg on Env.short, s"${outDof.name}.lastModified() > ${iniDof.name}.lastModified()") --
//    invBegin -- Skip(inv on Env.short, s"${invDof.name}.lastModified() > ${outDof.name}.lastModified()")
//  }
//}
