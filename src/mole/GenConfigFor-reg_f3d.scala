...
val csvFile = new java.io.File(if (args.length > 0) args(0) else "Config/reg_f3d.csv")
...
val write = Capsule(EmptyTask(), strainer = true) hook AppendToCSVFileHook(csvFile, ds, be, ln, maxit)
